...
function loadStatsPage() {
	...
	var win = 0;
	var loss = 0;
	for (row_counter = 2; col_counter < table.rows.length; row_counter++) {
		cell_value1 = table.rows[row_counter].cells[2].innerHTML;
		cell_value2 = table.rows[row_counter].cells[3].innerHTML;
		if (parseInt(cell_value1) > parseInt(cell_value2)) {
			winner = "CU Boulder";
			win = win + 1;
...
function loadPlayersPage() {
	...
	for (var player_count = 0, player; player = players[player_count]; player_count++) 
	{
		// dropdown.innerHTML = "<a class = 'dropdown-item' href = '#' onClick = 'switchPlayers(player_count)>"; 
		var load = document.createElement('a');
		load.innerHTML = player.name;
		load.href = '#';
		load.setAttribute('onclick' , 'switchPlayers(' + player_count + ');');
		dropdown.appendChild(load);
